...
function AffiliateLinks() {
    ...
    useEffect(async ()=>{
        ...
        let res1=await Api.getAffiliateLinkByUser(data);
        if (res1.status == '200')
        {
            setLinks(res1.data);
                ...
                <table id="example" className="table table-hover table-bordered">
                    ...
                    <tbody>
                    {
                        links?.map((itm,i)=>(
                            <tr>
                                <td>{itm.id}</td>
                                <td style={{cursor: 'pointer'}} onClick={()=>{Notifications.successMsg('Code Copied');navigator.clipboard.writeText(itm.code)}}>{itm.code}</td>
