...
function Cleaner() {
    ...
    let [cleaners,setCleaners]=useState(null);

    useEffect(async ()=>{

        let res=await Api.getAllCleaners();
        if (res.status == '200')
        {
    ...
    async function Delete(id) {
        ...
        let res= await Api.adminDeleteCleaner(data);
        console.log(res);
                ...
                <table id="example" className="table table-hover table-bordered">
                    ...
                    {cleaners?.map((itm,i)=>(
                        <tr>
                            ...
                            <td><i onClick={()=>{editCleaner(itm.id)}} style={{color:"green",cursor:"pointer"}} className={'fas fa-pen'}></i></td>
